...
export function part2(input: string): number {
  ...
  instructions.forEach((instruction) => {
    ...
    const elementsToMove = stackFrom.slice(
      stackFrom.length - cantelementsToMove,
      stackFrom.length
    );
    stackFrom.splice(stackFrom.length - cantelementsToMove, cantelementsToMove);
    stackTo.push(...elementsToMove);
